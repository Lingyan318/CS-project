...
create table FOREST (
    ...
    Constraint forest_UQ_name UNIQUE(name) deferrable initially immediate,
    Constraint forest_UQ_MBR UNIQUE(MBR_XMin, MBR_XMax, MBR_YMin, MBR_YMax) 
...
create table STATE (
    ...
    Constraint state_UQ_Name UNIQUE (Name) deferrable initially immediate
);
...
create table SENSOR (
	...
	initially deferred deferrable,
    Constraint sensor_UQ_coordinate UNIQUE(X,Y) deferrable initially immediate
);
...
--a
alter table sensor 
add constraint sensor_energy_check check(energy>=0 and energy<=10);


--b
alter table forest
add constraint forest_acid_level check(acid_level>=0 and acid_level<=1);


--c
alter table forest
add constraint forest_mbr check(mbr_xmin<mbr_xmax and mbr_ymin<mbr_ymax);


--d
alter table coverage
add constraint coverage_percentage1 check(percentage>=0 and percentage<=1);
